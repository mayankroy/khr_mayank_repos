...
void LoadWaypointList (vector<Waypoint>& waypoint_list_to_fill, string filepath_to_waypoints)
{
	...
	while (in.eof() != 1 )
	{
		...
		*/
		Waypoint newPoint;	
		//Create the point
		tStamp = getline(in,line,',');//atof(vec[tStamp_id-1].c_str());
		newPoint.x = atof(getline(in,line,','));//atof(vec[x_id-1].c_str()); 
		newPoint.y = atof(getline(in,line,','));//atof(vec[y_id-1].c_str());
		newPoint.yaw = atof(getline(in,line,','));//atof(vec[yaw_id-1].c_str());
